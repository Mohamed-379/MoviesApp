...
class OnlineDataSources
{
  ...
  static const String detailsScreen = "/3/movie/";
  ...
  {
    Uri url = Uri.parse("https://$urlBase$detailsScreen$id");
    Response response = await http.get(url, headers: headers);
    Map json = jsonDecode(response.body);
  ...
  {
    Uri url = Uri.parse("https://$simiBaseUrl$id$similarMovies");
    Response response = await http.get(url, headers: headers);
    Map json = jsonDecode(response.body);
    SimilarMoviesResponses similarMoviesResponses = SimilarMoviesResponses.fromJson(json);
    if(response.statusCode >= 200 && response.statusCode < 300 && similarMovies.isNotEmpty == true)
    {
